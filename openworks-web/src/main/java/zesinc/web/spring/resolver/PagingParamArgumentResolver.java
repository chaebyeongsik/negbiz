...
/**
 * <pre>
 * 검색 파라미터를 PageVO를 상속한 VO 객체의 paramMap에 자동설정한다.
 * 검색파라미터는 기본적으로 "q_"로 시작되며 환결설정 파일에서 변경이 가능하다.
 * 
 * 추가로 페이징 관련 파라미터를 자동으로 설정한다.
 ...
 */
public class PagingParamArgumentResolver extends ParamArgumentResolver {

    @Override
    public boolean supportsParameter(MethodParameter parameter) {

        return PageVO.class.isAssignableFrom(parameter.getParameterType());
    }
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
        ...
        try {
            ...
            // 페이징 정보를 설정 한다.
            setPageParam(webRequest, paramMap);

            // VO 클레스에 paramMap를 설정한다.
            setParamMap(webRequest, object, paramMap);

            logger.debug("PagingParamArgumentResolver object {} ", object);
        } catch (Exception e) {
