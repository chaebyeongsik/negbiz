...
/**
 * 기본 VO 객체로 페이징 관련 프로그램 이외에는 본 VO 객체를 상속받아 사용한다.
 * 
 * <pre>
...
public class BaseVO implements Serializable {
    ...
    /**
     * String 타입과 Integer 타입은 제외하고 이외의 타입 값을 사용하며,
     * Object를 반환하므로 적절히 케스틍하여 사용하도록 한다.
     * 
     * @param key
     * @return
     * @see {@link #getString(String) }
     * @see {@link #getStrings(String) }
     * @see {@link #getInteger(String) }
     * @see {@link #getIntegers(String) }
     */
    ...
    /**
     * String 문자열을 반환한다. 만약 값이 null 이라면 빈문자열("")을 반환하고,
     * 값이 String 타입이 아닌경우 toString() 메소드 호출 값을 반환한다.
     * 
     * @param key
     * @return
     */
     ...
     * <pre>
     * Integer 타입의 값을 반환한다. 
     * Integer 객체형이고 숫자 '0'도 의미가 있기 때문에 값이 없는 경우 null을 반환한다.
     * 
     * 문자열인 경우는 Integer로 케스팅 후 반환한다.
     * 따라서 케스팅을 할 수 없는 경우 {@link ClassCastException}}이 발생된다.
     * 
     * 이외의 경우 모두 null이 반환된다.
     * </pre>
     * 
     * @param key
     * @return
