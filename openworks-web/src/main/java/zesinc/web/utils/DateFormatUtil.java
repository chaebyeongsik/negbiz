 ...
 * 
 * ※ 참고
 * 1. TimeZone : 그리니치 표준시간(GMT)를 기준으로 각 지역이 위치한 경도에 따라
 * 시간의 차이가 있다. 이러한 동일한 시간대를 지역을 동일한 timezone을 가진다고 말한다.
 * 2. UTC(Universal Time Coordinated) : 지역 표준시를 말하며, GMT(그리니치 표준시)라고도 한다.
