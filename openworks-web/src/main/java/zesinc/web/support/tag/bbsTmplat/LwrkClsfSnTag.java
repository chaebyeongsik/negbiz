 ...
 * @see
 */
public class LwrkClsfSnTag extends BbsTagSupport {

    /**
     * 필드명 설정
     */
    public LwrkClsfSnTag() {
        this.fieldNm = "lwrkClsfSn";
    }
    ...
    public void doTag() throws JspException, IOException {
        ...
        if("list".equals(this.type)) {
            String lwrkClsfNm = getString("lwrkClsfNm");
            writer.print(lwrkClsfNm);
        } else if("view".equals(this.type)) {
            String lwrkClsfNm = getString("lwrkClsfNm");
            writer.print(lwrkClsfNm);
        } else if("value".equals(this.type)) {
            Integer lwrkClsfSn = getInteger();
            writer.print(lwrkClsfSn);
            ...
            Integer clsfNo = getInteger("clsfNo");
            Integer lwrkClsfSn = getInteger("lwrkClsfSn");
            ...
            if(Validate.isNotEmpty(ctgryList)) {
                html.append("<select id=\"").append(fieldNm).append("\" name=\"").append(fieldNm).append("\" class=\"")
                    .append(BbsStyleSupport.BBS_SELECT_CLASS).append("\">");
                for(BbsCtgryCacheVO bbsCtgryVo : ctgryList) {
                    if(bbsCtgryVo.getClsfNo().equals(clsfNo)) {
                        List<BbsCtgryCacheVO> lwprtCtgryList = bbsCtgryVo.getLwprtCtgryList();
                        if(Validate.isNotEmpty(lwprtCtgryList)) {
                            ...
                            for(BbsCtgryCacheVO lwprtCtgryVo : lwprtCtgryList) {
                                if(lwprtCtgryVo.getLwrkClsfSn().equals(lwrkClsfSn)) {
                                    html.append("<option value=\"").append(lwprtCtgryVo.getLwrkClsfSn()).append("\" selected=\"selected\">")
                                        .append(lwprtCtgryVo.getLwrkClsfNm()).append("</option>");
                                } else {
                                    html.append("<option value=\"").append(lwprtCtgryVo.getLwrkClsfSn()).append("\" >")
                                        .append(lwprtCtgryVo.getLwrkClsfNm()).append("</option>");
                                }
                            }
                        } else {
